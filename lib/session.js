...
// Transcript content is untrusted — it includes tool output, pasted text, and
// web-fetched text, any of which can carry raw ANSI/terminal escape sequences.
// Strip all C0/C1 control characters (incl. ESC 0x1b) before these strings ever
// reach the terminal in the picker. Removing the ESC byte neutralizes a sequence;
// any leftover printable chars (e.g. "[31m") are harmless literal text.
// eslint-disable-next-line no-control-regex
const CONTROL_CHARS = /[\x00-\x1f\x7f-\x9f]/g;
function stripControl(s) {
  return typeof s === 'string' ? s.replace(CONTROL_CHARS, ' ').replace(/\s+/g, ' ').trim() : s;
}

// Single-pass reader: title + real cwd + searchable text from one file read.
function readSessionInfo(filePath) {
  const empty = { customTitle: null, aiTitle: null, cwd: null, searchText: '' };
  ...
  // Strip control chars from every display-bound field. cwd is left raw: it's a
  // filesystem path used as a spawn cwd (realistically control-free), and altering
  // it could break resume.
  const searchText = stripControl(texts.join(' ')).slice(0, 4000);
  return {
    customTitle: customTitle === null ? null : (stripControl(customTitle) || null),
    aiTitle: aiTitle === null ? null : (stripControl(aiTitle) || null),
    cwd,
    searchText,
  };
}
...
function readSessionStats(filePath) {
  const empty = { messageCount: 0, firstTime: null, lastTime: null, durationMs: 0 };
  if (!filePath || !fs.existsSync(filePath)) return empty;
  let content;
  try { content = fs.readFileSync(filePath, 'utf8'); } catch { return empty; }
  let messageCount = 0;
  ...
  return {
    ...
    firstTime,
    lastTime,
    durationMs: (firstTime && lastTime) ? lastTime - firstTime : 0,
  };
}
...
function readSummary(sessionFilePath) {
  ...
  try {
    ...
    if (typeof msg === 'string') return stripControl(msg).slice(0, 80);
    if (Array.isArray(msg)) {
      const t = msg.find(b => b?.type === 'text')?.text;
      if (t) return stripControl(t).slice(0, 80);
    }
    if (typeof obj?.summary === 'string') return stripControl(obj.summary).slice(0, 80);
